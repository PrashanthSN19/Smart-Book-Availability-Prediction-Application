...
<body>
					...
					<ul class="navbar-nav ms-auto mb-2 mb-lg-0">
						...
						<li class="nav-item">
							<a class="nav-link fs-5 active fw-bold text-decoration-underline link-offset-2"
								href="mrequest.jsp">My Requests </a>
						</li>
						<li class="nav-item">
										...
										<% Connection con=null; 
											...
											<div class="table-responsive">
												...
												<table id="customers" class="table table-bordered">
													...
													<tbody>
														<% //session.setAttribute("Uname","Alan1208"); 
                                                                                                                    String uname=session.getAttribute("Uname").toString();
															if(uname.equals("")){ response.sendRedirect("login.jsp"); }
															//uname="Sunny Boyka6288" ; 
                                                                                                                        PreparedStatement st=con.prepareStatement("select Fid,Fname,ReqBy from request where Fowner='"+uname+"' and Stat='Pending'");
 res=st.executeQuery();
 while(res.next())
 {
 %>
 <tr>
     ...
     <td><a href=" approverequest.jsp?Fid=<%=res.getString(1)%>&Fname=<%=res.getString(2)%>&ReqBy=<%=res.getString(3)%>
																	" style="color:blue;">Approve</a>
																	</tr>
																	<% } %>
													</tbody>
												</table>
